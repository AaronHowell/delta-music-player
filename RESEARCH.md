...
delta_music_player/
    ...
    library/                   # xxx_notes.json / xxx_melody.mid / xxx_contour.json
    tests/                     # pytest
...
8. **Debug 能力**：任何音频输入都自动落盘 `library/<stem>_contour.json`（F0 轮廓）、`<stem>_notes.json`、`<stem>_melody.mid`（mido 写出，可用 DAW 检查）。
