...
这个项目的核心就一件事：**把 MIDI 映射成游戏乐器的按键序列，然后分毫不差地替你弹出来**。移调、单音化、修饰键组合，全都是为这一件事服务的。仓库里自带一首调好的《父亲》（`samples/father.mid`），装完就能直接开弹，见[快速上手](#快速上手gui)。

> 仅使用正常的 Windows 输入模拟 API（SendInput / MCI）：不修改游戏、不注入进程、不读写游戏内存、不绕过反作弊。

> ⚠️ **丑话说在前头**：反作弊系统怎么判定"模拟按键"，没有任何人能替你打包票——本工具不做任何手脚，但**用了仍有被判定违规、封号的可能，我们不保证不封号**。八个字：**用别怕，怕别用**。建议先拿小号、或者在输得起的环境里试。详见文末[免责声明](#免责声明)。
...
**没有现成乐谱？仓库自带一首能直接弹的《父亲》**（筷子兄弟）：`samples/father.mid`——我们从简谱逐音校对转录的单音主旋律（约 3 分钟、296 个音、钢琴音色试听）。在 GUI 里「选择 MIDI...」打开它，或者 CLI 一步到位：

```powershell
.venv\Scripts\python app.py samples\father.mid --dry-run    # 先看每个音按哪个键
.venv\Scripts\python app.py samples\father.mid              # 装备好乐器，直接开弹
```

这首曲子就是本项目想干的事的完整演示：一份普通 MIDI 进去，游戏乐器按键出来。你自己找的任何 `.mid` 也是同样的走法。

另外还有一首三轨演示曲（小星星，含旋律/伴奏/tempo，用来体验自动选轨）：
...
**先直接弹，听着不对再回来校准。** `config/instrument.json` 的出厂默认（基础行 C4-C5、修饰键 降调=-12 / 半音=+1 / 升调=+12、鼠标左/中/右键）是按常见布局给的，大部分情况下拿来就能用；只有当你发现**弹出来的音高不对**（比如整体偏高偏低、某个修饰键没效果）时，才需要进游戏实测校准：
...
想手动验证按键到不到位，可以用配套的手动测试短语（覆盖全部键型：八键/半音/升降调/双修饰/2秒长音/重复音）：
...
samples/                father.mid（《父亲》主旋律，可直接演奏）、twinkle.mid（演示曲）
tools/                  demo 生成、手动游戏内测试、输入诊断、MELODIA 探针、
                        GUI 性能基准（bench_ui.py）
tests/                  pytest（162 个）
...
- 在装有反作弊系统的在线游戏中使用任何自动化工具都可能违反游戏服务条款并导致账号处罚。**我们无法保证不被判定违规、不保证不封号**——用别怕，怕别用，风险由使用者自行承担。
- `samples/father.mid` 为我们按公开简谱人工转录的《父亲》（筷子兄弟）主旋律，仅作为功能演示；原曲版权归原作者所有，请勿用于任何商业用途。
