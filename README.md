...
> 仅使用正常的 Windows 输入模拟 API（SendInput / MCI）：不修改游戏、不注入进程、不读写游戏内存、不绕过反作弊。使用前请阅读文末[免责声明](#免责声明)。
...
没有现成乐谱？先生成一首演示曲（小星星，含旋律/伴奏/tempo 三轨）：
...
`config/instrument.json` 是按键映射的**唯一**事实来源。出厂默认是占位值（基础行假定为 C4-C5，修饰键假定 降调=-12 / 半音=+1 / 升调=+12 半音、鼠标左/中/右键），**进游戏实测后请校准**：
...
配套的手动测试短语（覆盖全部键型：八键/半音/升降调/双修饰/2秒长音/重复音）：
...
python app.py <input.mid|mp3|wav|flac>
    ...
    [--start-delay S]         倒计时秒数
    [--backend melodia|basic_pitch]  音频输入的主旋律提取后端
...
- 在装有反作弊系统的在线游戏中使用任何自动化工具都可能违反游戏服务条款并导致账号处罚，**风险由使用者自行承担**。
